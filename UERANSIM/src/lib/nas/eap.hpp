...
enum class EAttributeType : uint8_t
{
    ...
    //tahap-9 //Prime-FS
    AT_PUB_ECDHE = 152,  // ID-nya disamakan dengan Open5GS (jika kamu pakai 0x80 di sana)
    AT_KDF_FS = 153,      // (opsional) kalau kamu mau parsing AT_KDF_FS juga
    //end

    //tahap-10
    AT_PUB_HYBRID = 0xA0 // 160
    //end
...
class EapAttributes
{
    ...
    std::set<int> twoByteLengthAttributes{};


  public:
      std::string toString() const {
        std::ostringstream oss;
        // Menambahkan header untuk atribut
        oss << "EapAttributes: ";
        
        // Iterasi melalui atribut dan menambahkan setiap nilai ke string
        for (int i = 0; i < 256; ++i) {
            if (attributes[i].has_value()) {
                // Jika atribut ada, tambahkan informasi ke string
                oss << "Attribute " << i << ": " << attributes[i]->toHexString() << ", ";  // Ubah sesuai cara menampilkan atribut
            }
        }

        // Menambahkan informasi untuk urutan atribut yang diterima
        oss << "Order Received: ";
        for (const int& entry : order_received) {
            oss << entry << " ";
        }

        return oss.str();  // Mengembalikan string yang telah dibangun
    }
    bool isTwoByteLengthAttribute(EAttributeType key) const {
        return twoByteLengthAttributes.count(static_cast<int>(key)) > 0;
    }
    ...
    //tahap-9 //Prime-FS
    [[nodiscard]] OctetString getAttribute(EAttributeType type) const {
      int index = static_cast<int>(type);
      if (index < 0 || index >= 256) return {};
      if (attributes[index].has_value())
          return attributes[index].value().copy();  // PENTING: gunakan .copy()
      return {};
    }
    //end
  ...
  public:
    ...
    void putRawAttributeWith2ByteLength(EAttributeType key, OctetString &&value);
...
class EapEdhoc : public Eap
{
  public:
    OctetString rawData;

    EapEdhoc(ECode code, octet id);
    EapEdhoc(ECode code, octet id, OctetString &&rawData);
};

...
#include <iomanip>  // pastikan sudah include
...
static T *DecodeViaBuilder(const OctetView &stream)
{
    ...
    std::cerr << "[DecodeViaBuilder] Start decoding stream at offset "
    ...
    while (stream.hasNext())
    {
        ...
        if (builder.optionalDecoders.count(iei))
            ...
            int unknown_iei = stream.peekI();
            std::cerr << "[DecodeViaBuilder] Unknown IEI: 0x"
            << std::hex << unknown_iei
            << " at offset " << stream.currentIndex() << std::endl;

            throw std::runtime_error("Bad constructed NAS message: Unknown IEI");
        }
    }

    std::cerr << "[DecodeViaBuilder] Successfully decoded message." << std::endl;

...
IEEapMessage IEEapMessage::Decode(const OctetView &stream, int length)
{
    ...
    const uint8_t* subData = stream.rawData() + stream.currentIndex();
    OctetView subStream(subData, length);

    std::cerr << "============= [IEEapMessage::Decode] START =============" << std::endl;
    std::cerr << "[IEEapMessage::Decode] EAP IE length = " << length << std::endl;

    // Dump raw EAP data
    std::cerr << "[IEEapMessage::Decode] Raw EAP Data:" << std::endl;
    for (int i = 0; i < length; i++) {
        std::cerr << std::hex << std::setw(2) << std::setfill('0') << (int)subData[i] << " ";
        if ((i + 1) % 16 == 0) std::cerr << std::endl;
    }
    std::cerr << std::endl;

    // Decode EAP PDU
    r.eap = eap::DecodeEapPdu(subStream);

    if (!r.eap) {
        std::cerr << "[ERROR] r.eap is nullptr" << std::endl;
    }

    // Manual EAP Header dump
    if (length >= 6) {
        uint8_t code = subData[0];
        uint8_t id = subData[1];
        uint16_t len = (subData[2] << 8) | subData[3];
        uint8_t type = subData[4];
        uint8_t subtype = subData[5];

        std::cerr << "[EAP Header] code=" << (int)code
                  << ", id=" << (int)id
                  << ", len=" << len
                  << ", type=0x" << std::hex << (int)type
                  << ", subtype=0x" << std::hex << (int)subtype << std::dec << std::endl;

        int offset = 8; // EAP header + subtype + 2 reserved
        while (offset + 2 <= length) {
            //header 1byte
            // uint8_t ie_type = subData[offset];
            // uint8_t ie_len = subData[offset + 1];
            // int ie_total_len = ie_len * 4;

            // if (ie_total_len <= 0 || offset + ie_total_len > length) {
            //     std::cerr << "WARNING: IE overrun or invalid length at offset " << offset << std::endl;
            //     break;
            // }
            // //end
            ...
            uint8_t ie_type = subData[offset];
            uint16_t ie_len;
            if (ie_type == 0xA0 /* AT_PUB_HYBRID */) {
                // ie_len = subData[offset + 1] | subData[offset + 2];
                ie_len = (subData[offset + 1] << 8) | subData[offset + 2];
            }else{
                ie_len = subData[offset + 1];
            }
            int ie_total_len = ie_len * 4;

            std::cerr << "[EAP-IE] type=0x" << std::hex << (int)ie_type
                      << ", len=" << std::dec << (int)ie_len
                      << " (" << ie_total_len << " bytes)" << std::endl;

            std::cerr << "[EAP-IE] Content:" << std::endl;
            for (int i = 0; i < ie_total_len; i++) {
                std::cerr << std::hex << std::setw(2) << std::setfill('0')
                          << (int)subData[offset + i] << " "; 
                if ((i + 1) % 16 == 0) std::cerr << std::endl;
            }
            if (ie_total_len % 16 != 0) std::cerr << std::endl;
    ...
    std::cerr << "============= [IEEapMessage::Decode] END =============" << std::endl;

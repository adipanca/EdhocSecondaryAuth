...
OctetString CalculatePrfPrime(const OctetString &key, const OctetString &input, int outputLength)
{
    ...
    for (int i = 0; i < round; i++)
    {
        ...
        // --- PRINT buffer sebelum HMAC ---
        std::cout << "[PRF'] Round " << (i + 1) << " Input to HMAC (hex): " 
        << s.toHexString() << std::endl;

        T[i] = HmacSha256(key, s);

        // --- PRINT buffer sesudah HMAC ---
        std::cout << "[PRF'] Round " << (i + 1) << " Output HMAC (T[" << i << "]) (hex): " 
        << T[i].toHexString() << std::endl;
    }
...
//tahap-9 //Prime-FS
OctetString CalculatePrfPrimeFS(const OctetString &key, const OctetString &input, int outputLength)
{
    if (key.length() != 48)
        throw std::runtime_error("CalculatePrfPrime, 256-bit key expected");
...
OctetString HmacSha256(const OctetString &key, const OctetString &input)
{
    ...
    std::cout << "[HMAC] Key   : " << key.toHexString() << std::endl;
    std::cout << "[HMAC] Key Length : " << key.length() << std::endl;
    std::cout << "[HMAC] Input   : " << input.toHexString() << std::endl;
    std::cout << "[HMAC] Input Length : " << input.length() << std::endl;
...
//     std::cout << "[HMAC] Output: " 
//               << OctetString::FromArray(out.data(), out.size()).toHexString() 
//               << std::endl;
...
OctetString CalculateKdfKey(const OctetString &key, int fc, OctetString *parameters, int numberOfParameter)
{
    ...
    std::cout << "[KDF] FC: " << std::hex << fc << std::dec << std::endl;
    
    for (int i = 0; i < numberOfParameter; i++)
    {
        std::cout << "[KDF] Parameter[" << i << "]: " << parameters[i].toHexString() << std::endl;
        inp.append(parameters[i]);

        uint16_t len = parameters[i].length();
        inp.appendOctet2(len);
        std::cout << "[KDF] Parameter[" << i << "] len: " << len << " (appended as 2 bytes)" << std::endl;
    }

    std::cout << "[KDF] Final Input to HMAC: " << inp.toHexString() << std::endl;

    OctetString result = HmacSha256(key, inp);
    std::cout << "[KDF] Output HMAC-SHA256: " << result.toHexString() << std::endl;
...
OctetString EncodeKdfString(const std::string &string)
{
    auto vec = std::vector<uint8_t>{string.c_str(), string.c_str() + string.length()};
    OctetString encoded{std::move(vec)};  // <-- fix pakai std::move

    std::cout << "[KDF-Encode] SNN (raw string): " << string << std::endl;
    std::cout << "[KDF-Encode] SNN (hex): " << encoded.toHexString() << std::endl;

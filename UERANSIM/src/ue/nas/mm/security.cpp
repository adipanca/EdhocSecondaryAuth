...
void NasMm::receiveSecurityModeCommand(const nas::SecurityModeCommand &msg)
{
    ...
    if (msg.selectedNasSecurityAlgorithms.integrity != nas::ETypeOfIntegrityProtectionAlgorithm::IA0)
    {
        ...
        if (calculatedMac != static_cast<uint32_t>(msg._macForNewSC))
        {
            ...
            m_logger->info("=== [ComputeMac DEBUG] Parameters ===");
            m_logger->info("Integrity Alg      : %d", static_cast<int>(tmpCtx.integrity));
            m_logger->info("Downlink NAS Count : %u", tmpCtx.downlinkCount);
            m_logger->info("is3gppAccess       : %s", tmpCtx.is3gppAccess ? "true" : "false");
            m_logger->info("isNewContext       : %s", "false");  // hardcoded sesuai argumen
            m_logger->info("KNASint            : %s", tmpCtx.keys.kNasInt.toHexString().c_str());
            m_logger->info("Plain NAS PDU      : %s", msg._originalPlainNasPdu.toHexString().c_str());

            m_logger->info("Calculated MAC     : %08X", calculatedMac);
            m_logger->info("Received MAC       : %08X", static_cast<uint32_t>(msg._macForNewSC));

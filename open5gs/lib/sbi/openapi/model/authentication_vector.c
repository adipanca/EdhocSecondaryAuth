...
void OpenAPI_authentication_vector_free(OpenAPI_authentication_vector_t *authentication_vector)
{
    ...
    //tahap-2
    if (authentication_vector->k_aut) {
        ogs_free(authentication_vector->k_aut);
        authentication_vector->k_aut = NULL;
    }
    // if (authentication_vector->eap_family) {
    //     ogs_free(authentication_vector->eap_family);
    //     authentication_vector->eap_family = NULL;
    // }
    //end
...
cJSON *OpenAPI_authentication_vector_convertToJSON(OpenAPI_authentication_vector_t *authentication_vector)
{
    ...
    // //tahap-2
    //penggunaan IF belum tepat, jadi matikan dulu kalau EAP dan kalau 5G_AKA cari solusi
    // if(authentication_vector->av_type == OpenAPI_auth_type_EAP_AKA_PRIME){
        if (authentication_vector->k_aut) {
            if (cJSON_AddStringToObject(item, "k_aut", authentication_vector->k_aut) == NULL) {
                ogs_error("OpenAPI_authentication_vector_convertToJSON() failed [k_aut]");
                goto end;
            }
        }
        // if (authentication_vector->eap_family) {
        //     if (cJSON_AddStringToObject(item, "eap_family", authentication_vector->eap_family) == NULL) {
        //         ogs_error("OpenAPI_authentication_vector_convertToJSON() failed [eap_family]");
        //         goto end;
        //     }
        // }
    // }
...
OpenAPI_authentication_vector_t *OpenAPI_authentication_vector_parseFromJSON(cJSON *authentication_vectorJSON)
{
    ...
    cJSON *kausf = NULL;
    cJSON *k_aut= NULL; //tahap-2
    // cJSON *eap_family= NULL; //tahap-9 //Prime-FS
    ...
    //tahap-2
    //penggunaan IF belum tepat, jadi matikan dulu kalau EAP dan kalau 5G_AKA cari solusi
    // if(av_typeVariable == OpenAPI_auth_type_EAP_AKA_PRIME){
        k_aut = cJSON_GetObjectItemCaseSensitive(authentication_vectorJSON, "k_aut");
        if (k_aut) {
        if (!cJSON_IsString(k_aut) && !cJSON_IsNull(k_aut)) {
            ogs_error("OpenAPI_authentication_vector_parseFromJSON() failed [k_aut]");
            goto end;
        }
        }
        // eap_family = cJSON_GetObjectItemCaseSensitive(authentication_vectorJSON, "eap_family");
        // if (eap_family) {
        // if (!cJSON_IsString(eap_family) && !cJSON_IsNull(eap_family)) {
        //     ogs_error("OpenAPI_authentication_vector_parseFromJSON() failed [eap_family]");
        //     goto end;
        // }
        // }
    // }
    ...
    // if (av_typeVariable == OpenAPI_auth_type_EAP_AKA_PRIME) {
        if(k_aut){
            authentication_vector_local_var->k_aut = k_aut && !cJSON_IsNull(k_aut) ? ogs_strdup(k_aut->valuestring) : NULL;
        }
        // if(eap_family){
        //     authentication_vector_local_var->eap_family = eap_family && !cJSON_IsNull(eap_family) ? ogs_strdup(eap_family->valuestring) : NULL;
        // }
    // }
    //end

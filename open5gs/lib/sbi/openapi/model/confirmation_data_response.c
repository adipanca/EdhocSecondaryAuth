...
cJSON *OpenAPI_confirmation_data_response_convertToJSON(OpenAPI_confirmation_data_response_t *confirmation_data_response)
{
    ...
    //tahap-6
    if (confirmation_data_response->eap_payload_b64) {
    if (cJSON_AddStringToObject(item, "eapPayloadB64", confirmation_data_response->eap_payload_b64) == NULL) {
        ogs_error("OpenAPI_confirmation_data_response_convertToJSON() failed [eap_payload_b64]");
        goto end;
    }
    }
    //end
...
OpenAPI_confirmation_data_response_t *OpenAPI_confirmation_data_response_parseFromJSON(cJSON *confirmation_data_responseJSON)
{
    ...
    // //tahap-6
    char *eap_payload_b64_str = NULL;
    cJSON *eap_payload_b64 = cJSON_GetObjectItemCaseSensitive(confirmation_data_responseJSON, "eapPayloadB64");
    if (eap_payload_b64 && cJSON_IsString(eap_payload_b64))
        eap_payload_b64_str = ogs_strdup(eap_payload_b64->valuestring);
    // //end
    ...
    //tahap-6
    confirmation_data_response_local_var->eap_payload_b64 = eap_payload_b64_str;
    //end

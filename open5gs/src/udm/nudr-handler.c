    ...
    udm_ue_t *udm_ue, ogs_sbi_stream_t *stream, ogs_sbi_message_t *recvmsg)
{
    ...
    char xres_string[OGS_KEYSTRLEN(OGS_MAX_RES_LEN)]; //tahap-1
    ...
    CASE(OGS_SBI_RESOURCE_NAME_AUTHENTICATION_SUBSCRIPTION)
        ...
        CASE(OGS_SBI_HTTP_METHOD_PATCH)
            ...
            memset(udm_ue->k_aut, 0, sizeof(udm_ue->k_aut));  // Pastikan buffer kosong
            ...
            //tahap-1
            if(udm_ue->auth_type == OpenAPI_auth_type_5G_AKA){
                ...
                //tahap-1
                uint8_t k_aut[32];       // K_aut = MK[16..47]
                char k_aut_string[OGS_KEYSTRLEN(32)];
                ogs_hex_to_ascii(k_aut, 32, k_aut_string, sizeof(k_aut_string));
                AuthenticationVector.k_aut = ogs_strdup(k_aut_string);
                //end

            }else{
                // Logging
                ogs_info("[%s] Generated AV EAP-AKA'", udm_ue->suci);

                // HEX-encode all fields (NULL-terminated)
                ogs_hex_to_ascii(udm_ue->rand, OGS_RAND_LEN, rand_string, sizeof(rand_string));
                ogs_hex_to_ascii(autn, OGS_AUTN_LEN, autn_string, sizeof(autn_string));
                ogs_hex_to_ascii(xres, sizeof(xres), xres_string, sizeof(xres_string));
                ogs_hex_to_ascii(xres_star, sizeof(xres_star), xres_star_string, sizeof(xres_star_string));


                AuthenticationVector.rand = ogs_strdup(rand_string);
                AuthenticationVector.autn = ogs_strdup(autn_string);
                AuthenticationVector.xres = ogs_strdup(xres_string);
                AuthenticationVector.xres_star = ogs_strdup(xres_star_string);

                // ===== EAP-AKA' Specific: Derive CK', IK', MK, and K_aut =====
                uint8_t ck_prime[OGS_KEY_LEN];
                uint8_t ik_prime[OGS_KEY_LEN];
                uint8_t mk[208];         //  208 bytes sesuai RFC 5448
                uint8_t k_aut[32];       // K_aut = MK[16..47]
                uint8_t kausf[32];       // Left(256, EMSK)


                uint8_t sqn_xor_ak[6];
                int i;
                for (i = 0; i < 6; i++) {
                    sqn_xor_ak[i] = udm_ue->sqn[i] ^ ak[i];
                }

                print_hex("SQN", udm_ue->sqn, 6);
                ogs_info("SNN = %s", udm_ue->serving_network_name);
                print_hex("AK", ak, 6);
                print_hex("SQN xor AK", sqn_xor_ak, 6);
                print_hex("IK = ", ik, 16);
                print_hex("CK = ", ck, 16);

                ogs_kdf_ckprime_ikprime(ck, ik, udm_ue->serving_network_name, sqn_xor_ak, ck_prime, ik_prime);

                ogs_assert(udm_ue->supi && strlen(udm_ue->supi) > 0);
                // const uint8_t *identity = (const uint8_t *)udm_ue->supi;
                // size_t identity_len = strlen(udm_ue->supi);

                //tanpa imsi- karena di ueransim tanpa imsi-
                const char *supi = udm_ue->supi;           // "imsi-999700000000001"
                ...
                // Derive MK = PRF′(IK'||CK', "EAP-AKA'" | SUPI)
                // ogs_kdf_prf_prime(ik_prime, ck_prime, identity, identity_len, mk);
                // tanpa imsi-
                ogs_kdf_prf_prime(ik_prime, ck_prime, (const uint8_t *)identity, identity_len, mk);


                // Assign K_aut ← MK[16..47]
                memcpy(k_aut, mk + 16, 32);
                memcpy(udm_ue->k_aut, k_aut, 32);

                // Optional debug print
                print_hex("IK'", ik_prime, 16);
                print_hex("CK'", ck_prime, 16);
                print_hex("MK", mk, 208);
                print_hex("K_AUT", k_aut, 32);

                // Derive KAUSF = MK[144..175] (from EMSK portion)
                memcpy(kausf, mk + 144, 32);  // 32 byte = 256 bit || KAUSF = Left(256, EMSK)

                // Logging
                print_hex("KAUSF", kausf, 32);

                // Simpan ke AuthenticationVector.ck_prime sebagai string hex
                char ck_prime_string[OGS_KEYSTRLEN(32)];
                ogs_hex_to_ascii(ck_prime, 32, ck_prime_string, sizeof(ck_prime_string));
                AuthenticationVector.ck_prime = ogs_strdup(ck_prime_string);

                // Simpan ke AuthenticationVector.ck_prime sebagai string hex
                char ik_prime_string[OGS_KEYSTRLEN(32)];
                ogs_hex_to_ascii(ik_prime, 32, ik_prime_string, sizeof(ik_prime_string));
                AuthenticationVector.ik_prime = ogs_strdup(ik_prime_string);

                // Simpan ke AuthenticationVector.k_ausf sebagai string hex
                char kausf_string[OGS_KEYSTRLEN(32)];
                ogs_hex_to_ascii(kausf, 32, kausf_string, sizeof(kausf_string));
                AuthenticationVector.kausf = ogs_strdup(kausf_string);
                ...
                ogs_info("k_aut_string = %s", k_aut_string);

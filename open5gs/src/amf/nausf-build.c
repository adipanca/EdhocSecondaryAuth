        ...
        amf_ue_t *amf_ue, void *data)
{
    ...
    //tahap-6
    ConfirmationData->auth_type = OpenAPI_auth_type_5G_AKA;
...
//tahap-6
ogs_sbi_request_t *amf_nausf_auth_handle_eap_response(
    amf_ue_t *amf_ue, void *data)
{
    ogs_sbi_message_t message;
    ogs_sbi_request_t *request = NULL;
    ...
    ogs_assert(amf_ue);
    ogs_assert(amf_ue->eap_session_href);

    if (amf_ue->eap_response && amf_ue->eap_response_len > 0) {
        int out_len = ((amf_ue->eap_response_len + 2) / 3) * 4;
        encoded_buf = ogs_malloc(out_len + 1);
        ogs_assert(encoded_buf);

        encoded_len = ogs_base64_encode(
            encoded_buf,
            (const char *)amf_ue->eap_response,
            amf_ue->eap_response_len
        );
        ogs_assert(encoded_len > 0);
        encoded_buf[encoded_len] = '\0';
    }

    memset(&message, 0, sizeof(message));
    message.h.method = (char *)OGS_SBI_HTTP_METHOD_PUT;
    message.h.uri = amf_ue->eap_session_href;
    ...
    ConfirmationData->auth_type = OpenAPI_auth_type_EAP_AKA_PRIME;
    ConfirmationData->eap_payload = encoded_buf ? ogs_strdup(encoded_buf) : NULL;

    ConfirmationData->res_star = xres_star_string;

    message.ConfirmationData = ConfirmationData;

    ogs_info("ConfirmationData SUCCESS");

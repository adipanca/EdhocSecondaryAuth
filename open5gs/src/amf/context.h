...
struct amf_ue_s {
    ...
    //tahap-3
    OpenAPI_auth_type_e auth_type; // <-- Tambahkan ini
    // Tambahkan ini untuk dukung EAP-AKA'
    char *eap_session_href;    // Untuk menyimpan URI ke eap-session (opsional)
    uint8_t *eap_payload;
    int eap_payload_len;
    int ngKSI;  // Tambahkan ini untuk menyimpan nilai ngKSI
    //end

    //tahap-6
    uint8_t *eap_response;
    int eap_response_len;
    char *eap_success;
    //end

...
int gmm_handle_authentication_response_eap(
    amf_ue_t *amf_ue,
    ogs_nas_5gs_authentication_response_t *authentication_response)
{
    int r;
    ...
    ogs_debug("[%s] Handling EAP-AKA' Authentication Response gmm_handle_authentication_response_eap", amf_ue->suci);

    CLEAR_AMF_UE_TIMER(amf_ue->t3560);
    
    ogs_nas_eap_message_t *eap_message = &authentication_response->eap_message;

    if (!eap_message->buffer || eap_message->length <= 0) {
        ogs_error("[%s] Missing or invalid EAP Response", amf_ue->suci);
        return OGS_ERROR;
    }

    print_hex("EAP-Response (binary)", eap_message->buffer, eap_message->length);

    // Simpan EAP response dari UE
    if (amf_ue->eap_response)
        ogs_free(amf_ue->eap_response);
    amf_ue->eap_response = eap_message->buffer;
    amf_ue->eap_response_len = eap_message->length;

    ogs_info("[%s] Received EAP-Response [%d bytes]",
        amf_ue->suci,
        eap_message->length);
    ...
    r = amf_ue_sbi_discover_and_send(
        OGS_SBI_SERVICE_TYPE_NAUSF_AUTH, NULL,
        amf_nausf_auth_handle_eap_response, amf_ue, 0, NULL);
    ogs_expect(r == OGS_OK);
    ogs_assert(r != OGS_ERROR);

        ...
        ran_ue_t *ran_ue, ogs_pkbuf_t *pkbuf)
{
    ...
    ogs_info("ngap_build_downlink_nas_transport() succeess");
    if (!ngapbuf) {
    ...
    ogs_info("ngap_send_to_ran_ue() succeess");
    ogs_expect(rv == OGS_OK);
...
int nas_5gs_send_de_registration_accept(amf_ue_t *amf_ue)
{
    ...
    ogs_info("[%s] masuk sini nas_5gs_send_de_registration_accept", amf_ue->suci);
...
int nas_5gs_send_authentication_request(amf_ue_t *amf_ue)
{
    ...
    if (amf_ue->auth_type == OpenAPI_auth_type_EAP_AKA_PRIME) {
        // Gunakan builder khusus untuk EAP
        if (!amf_ue->eap_payload || amf_ue->eap_payload_len == 0) {
            ogs_error("[%s] EAP payload kosong", amf_ue->suci);
            return OGS_ERROR;
        }
        
        gmmbuf = gmm_build_eap_auth_request(amf_ue);
        if (!gmmbuf) {
            ogs_error("gmm_build_eap_auth_request() failed");
            return OGS_ERROR;
        }
    } else {
    ...
    ogs_info("Siap Kirim nas_5gs_send_to_downlink_nas_transport");
    rv = nas_5gs_send_to_downlink_nas_transport(ran_ue, gmmbuf);
    ogs_expect(rv == OGS_OK);
    ogs_info("Siap Kirim rv == OGS_OK");
    return rv;
}
...
int nas_5gs_send_authentication_result(amf_ue_t *amf_ue)
{
    ...
    // uint8_t eap_payload[512];
    uint8_t eap_payload[2048]; 
    int eap_payload_len = 0;

    ogs_assert(amf_ue);
    ogs_assert(amf_ue->eap_response);

    // Decode base64 string (EAP-Success) ke binary
    eap_payload_len = ogs_base64_decode(
        (char *)eap_payload,
        (const char *)amf_ue->eap_response);

    if (eap_payload_len <= 0 || eap_payload_len > 255) {
        ogs_error("[%s] Invalid or too large EAP payload (len=%d)", amf_ue->suci, eap_payload_len);
        return OGS_ERROR;
    }

    ogs_info("[%s] Sending NAS Authentication Result with EAP IE", amf_ue->suci);

    // 5GS NAS PLAIN header
    buffer[offset++] = 0x7E;        // Security header type: plain NAS message
    buffer[offset++] = 0x02;        // Message type: Authentication Result (0x02)

    // EAP Message IE (0x78)
    buffer[offset++] = 0x78;        // IEI for EAP Message
    buffer[offset++] = eap_payload_len;  // Length (1 byte)
    memcpy(&buffer[offset], eap_payload, eap_payload_len);
    offset += eap_payload_len;

    // Buat pkbuf untuk dikirim ke RAN
    pkbuf = ogs_pkbuf_alloc(NULL, offset);
    ogs_assert(pkbuf);
    memcpy(pkbuf->data, buffer, offset);
    pkbuf->len = offset;

    return nas_5gs_send_to_gnb(amf_ue, pkbuf);
}
